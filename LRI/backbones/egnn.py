...
class EGNN(nn.Module):
    ...
    def forward(self, x, pos, edge_attr, edge_index, batch, edge_attn=None, node_attn=None, att_opt="no"):
        ...
        if att_opt == "mean":
            all_feats = []
            for seli in range(self.n_layers):
                xx = x.clone()
                for i in range(self.n_layers):
                    identity = xx
                    if i!=seli:
                        xx, _, _ = self.convs[i](xx, edge_index, pos, batch=batch, edge_attr=edge_attr)
                    else:
                        xx, _, _ = self.convs[i](xx, edge_index, pos, batch=batch, edge_attr=edge_attr, edge_attn=edge_attn)
                    xx = xx + identity
        ...
        for i in range(self.n_layers):
            identity = x
            if (att_opt.lower()=="first" and i!=0) or (att_opt.lower()=="last" and i!=self.n_layers-1):
                x, _, _ = self.convs[i](x, edge_index, pos, batch=batch, edge_attr=edge_attr)
            else:
...
class E_GCL_mask(E_GCL):
    ...
    def forward(self, h, edge_index, coord, batch, edge_attr=None, node_attr=None, edge_attn=None, last_att=False):
        row, col = edge_index
        radial, coord_diff = self.coord2radial(edge_index, coord)
