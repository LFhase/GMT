...
def update_and_save_best_epoch_res(baseline, train_res, valid_res, test_res, metric_dict, epoch, model_dir, seed, topk, warmup, writer, args, config=None,clf=None,extractor=None):
    ...
    if better_val_auc or same_val_auc_but_better_val_loss:
        ...
        if config is not None and config['save_mcmc']:
            # mcmc_dir = Path("/".join(str(self.model_dir).split("/")[:3]))
            save_checkpoint(clf, config['mcmc_dir'], model_name=config['pre_model_name']+f"_clf_mcmc")
            save_checkpoint(extractor, config['mcmc_dir'], model_name=config['pre_model_name']+f"_att_mcmc")
